...
kpi_box(s, "分析対象者数（課題3）", "35名", 0.3, 1.2)
kpi_box(s, "全体平均スコア（課題3）", "84.9点", 2.95, 1.2)
kpi_box(s, "分析対象者数（課題2）", "5名", 5.6, 1.2)
kpi_box(s, "期間中の全体成長幅", "+9.4点", 8.25, 1.2)
kpi_box(s, "最大成長者", "田中一郎 +9.3点", 10.9, 1.2, val_col=GREEN)

findings = [
    (0, "【課題3：所属別パフォーマンス】", True, NAVY),
...
s = add_slide()
bg(s)
header(s, "課題3｜所属別参加者数", "ドーナツ型円グラフ")
...
s = add_slide()
bg(s)
header(s, "課題3｜所属別スコア比較", "平均・最高・最低 グループ棒グラフ")
...
s = add_slide()
bg(s)
header(s, "課題3｜スコア分布ヒストグラム", "所属別色分け＋全体平均線（84.9点）")
...
s = add_slide()
bg(s)
header(s, "課題3｜マーケティング分析サマリー", "強み・課題・リスクの三層構造")
...
s = add_slide()
bg(s)
header(s, "課題2｜日別スコア成長トレンド", "受講者別折れ線グラフ　／　全体平均（破線）")
...
s = add_slide()
bg(s)
header(s, "課題2｜受講者×科目 スコアヒートマップ", "赤（低）→黄（中）→緑（高）のグラデーション")
...
s = add_slide()
bg(s)
header(s, "課題2｜スコア分布・成長幅ランキング", "箱ひげ図（分布）＋横棒グラフ（初日→最終日の変化）")
...
s = add_slide()
bg(s)
header(s, "課題2｜受講者別パフォーマンス詳細", "平均・成長幅・弱点・推奨施策")
